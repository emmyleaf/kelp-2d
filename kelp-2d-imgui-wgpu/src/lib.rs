...
impl ImGuiRenderer {
    ...
    pub fn new(font_texture: &mut FontTexture, device: &Device, queue: &Queue, config: RendererConfig) -> Self {
        ...
        renderer.reload_font_texture(font_texture, device, queue);
    ...
    pub fn prepare(
        ...
        for draw_list in draw_data.draw_lists() {
            render_data.draw_list_offsets.push((vertex_count as i32, index_count as u32));
        ...
        indices
            .resize(indices.len() + COPY_BUFFER_ALIGNMENT as usize - indices.len() % COPY_BUFFER_ALIGNMENT as usize, 0);
    ...
    pub fn split_render<'r>(
        ...
        rpass.set_index_buffer(render_data.index_buffer.as_ref().unwrap().slice(..), IndexFormat::Uint16);

        // Execute all the imgui render work.
        for (draw_list, bases) in draw_data.draw_lists().zip(render_data.draw_list_offsets.iter()) {
    ...
    fn render_draw_list<'render>(
        ...
        for cmd in draw_list.commands() {
            if let Elements { count, cmd_params } = cmd {
                ...
                let tex = self.textures.get(texture_id).ok_or(RendererError::BadTexture(texture_id))?;
    ...
    pub fn reload_font_texture(&mut self, font_texture: &mut FontTexture, device: &Device, queue: &Queue) {
        // Remove possible font atlas texture.
        if let Some(id) = font_texture.tex_id {
